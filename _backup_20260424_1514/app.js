...
const FL_DEFAULTS = {
  ...
  SUPABASE_ANON_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVkZ251ZHJieXN5YmVmYnF5aWpxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyNTExNjEsImV4cCI6MjA5MTgyNzE2MX0.UOTH1J-022hwSQZ2QkpiRxw3wtctaVsJQEBoLYYMkHk',
...
if (!localStorage.getItem('fl_admin_key')) {
  localStorage.setItem('fl_admin_key', 'b8464678b573c885c449958a9ea760c08b01279d01d3a1f996fc92b7364f10b7');
}
...
function getDayNumber() {
  const start = new Date(FL.STREAK_START + 'T00:00:00');
  const now = new Date();
...
function getEffectiveToday() {
  // Use LOCAL date, not UTC — toISOString() returns UTC which is 5:30h behind IST
  var now = new Date();
...
function addToSyncQueue(table, data, query) {
  ...
  // Deduplicate: replace existing entry for same table+key
...
async function pushAllToSupabase() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key && key.startsWith('fl_daily_rule_read_')) {
      ...
      if (typeof syncReadingLog === 'function') { await syncReadingLog(rDate, 'daily_rule', null); count++; }
    }
  }
...
const CM_REACTION_EMOJI = { fire: '\uD83D\uDD25', clap: '\uD83D\uDC4F', strong: '\uD83D\uDCAA', crown: '\uD83D\uDC51' };
let cmOffset = 0;
let cmReplyTo = null;
...
async function loadComments() {
  if (!SB.init()) return;
  var comments = await sbFetch('comments', 'GET', null, '?order=created_at.desc&limit=' + CM_PAGE_SIZE + '&offset=' + cmOffset);
  ...
  (reactions || []).forEach(function(r) {
    if (!rCounts[r.comment_id]) rCounts[r.comment_id] = { fire:0, clap:0, strong:0, crown:0 };
...
function renderComment(c, counts, myReaction, isMilestone, isReply) {
  var avatarBg = c.visitor_id === 'system' ? 'var(--cyan)' : cmAvatarColor(c.display_name);
  var initial = (c.display_name || '?')[0].toUpperCase();
  var cls = 'cm-comment' + (isMilestone ? ' cm-milestone' : '');
  var html = '<div class="' + cls + '" data-cid="' + c.id + '">';
  html += '<div class="cm-header">';
  html += '<div class="cm-avatar" style="background:' + avatarBg + '">' + (isMilestone ? '\uD83C\uDFC6' : initial) + '</div>';
  html += '<div><span class="cm-name">' + (c.display_name || 'Anonymous') + '</span><span class="cm-time">' + cmTimeAgo(c.created_at) + '</span></div>';
  html += '</div>';
  html += '<div class="cm-body">' + (c.content || '').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div>';
  html += '<div class="cm-actions">';
  ['fire','clap','strong','crown'].forEach(function(r) {
...
function postComment() {
  ...
  if (!text) return;
  if (!identity) { cmPendingComment = text; showIdentityModal(); return; }
  _doPostComment(text);
}
...
async function _doPostComment(text) {
  ...
  if (Date.now() - lastTime < 60000) { alert('Please wait ' + Math.ceil((60000 - (Date.now() - lastTime)) / 1000) + ' seconds before posting again.'); return; }
  var identity = getVisitorIdentity();
  var content = text.replace(/<[^>]*>/g, '').substring(0, 500);
  var btn = document.getElementById('cmPostBtn');
  btn.disabled = true; btn.textContent = 'POSTING...';
  var result = await cmInsert('comments', {
...
function showReplyInput(commentId) {
  ...
  if (slot) {
    slot.innerHTML = '<div class="cm-reply-input"><textarea rows="2" placeholder="Reply..." maxlength="500" id="cmReplyText"></textarea><button onclick="postReply()">REPLY</button><button onclick="cancelReply()" style="background:transparent;color:var(--text-dim);border:1px solid var(--surface-border)">X</button></div>';
