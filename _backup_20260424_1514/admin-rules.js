...
function toggle25Rule(ruleNum) {
  ...
  // Check if all 25 complete → sync to reading_log
  var readCount = Object.keys(state).filter(function(k) { return state[k]; }).length;
  if (readCount >= 25 && typeof syncReadingLog === 'function') {
    syncReadingLog(getEffectiveToday(), 'full_25', null);
  }
}
...
function markAll25Read() {
  ...
  renderRules();
  markSaved();
  if (typeof syncReadingLog === 'function') {
    syncReadingLog(getEffectiveToday(), 'full_25', null);
  }
}
