...
async function backfillStravaCalories(log: string[], limit: number) {
 ...
 const { count: totalRemaining } = await supaAdmin.from('strava_activities').select('id', { count: 'exact', head: true }).is('calories_synced_at', null)
 ...
 const { data: rows, error } = await supaAdmin.from('strava_activities').select('id, name, start_date_local, type, moving_time').is('calories_synced_at', null).order('start_date_local', { ascending: false }).limit(limit)
...
async function migrateImagesToStorage(log: string[], igToken: string) {
 ...
 const { data: cdnPosts } = await supaAdmin.from('instagram_posts').select('id,ig_id,media_url,day_number').not('media_url', 'like', '%supabase%').not('media_url', 'is', null).order('day_number', { ascending: false }).limit(15)

 const { data: nullPosts } = await supaAdmin.from('instagram_posts').select('id,ig_id,media_url,day_number').is('media_url', null).order('day_number', { ascending: false }).limit(10)
...
async function syncProofForDate(targetDate: string, log: string[]) {
 ...
 const { data: dayActivities } = await supaAdmin.from('strava_activities').select('*').gte('start_date_local', targetDate + 'T00:00:00').lt('start_date_local', targetDate + 'T23:59:59')

 // Check existing proof_archive row
 const { data: existing } = await supaAdmin.from('proof_archive').select('sleep_hrs,food_clean,gym,run_km,cycle_km,swim_km').eq('date', targetDate).maybeSingle()
...
async function healthIngest(body: Record<string, unknown>): Promise<{ success: boolean; ingested: number; dates_processed: string[]; errors?: Array<{date: string; error: string}> }> {
 ...
 const { data: existingRow } = await supaAdmin.from('health_daily').select('sleep_hours,vo2_max,workout_count,workout_types,workout_total_min,workout_total_cal,raw_payload').eq('date', date).maybeSingle()
...
async function _todayRunStats(): Promise<{ km: number; min: number; pace: string; start: string; name: string } | null> {
 ...
 const { data } = await supaAnon.from('strava_activities').select('name,type,start_date_local,distance,moving_time').eq('type', 'Run').gte('start_date_local', today + 'T00:00:00').lt('start_date_local', today + 'T23:59:59').order('start_date_local', { ascending: false }).limit(1)
...
<div style="font-family:'Courier New',monospace;font-size:11px;color:rgba(212,168,67,0.55);letter-spacing:1px">COMMITMENT · LOGGED PUBLIC · EVERY MORNING</div>
...
<p>If the deadline passed without a run, the slip should be logged public and paid out. The system does not negotiate.</p>
...
<tr><td style="border-bottom:1px dashed rgba(212,168,67,0.2);color:rgba(212,168,67,0.6);font-size:12px">COMMITMENT</td><td style="border-bottom:1px dashed rgba(212,168,67,0.2);text-align:right;font-size:18px;color:#D4A843"> DEFENDED</td></tr>
...
async function emailWeeklyRecap() {
 ...
 const { data } = await supaAnon.from('strava_activities').select('distance,moving_time,start_date_local,type').gte('start_date_local', sevenDaysAgo + 'T00:00:00').eq('type', 'Run')
 const runs = data || []
...
<tr><td style="border-bottom:1px dashed rgba(212,168,67,0.2);color:rgba(212,168,67,0.6);font-size:12px">COMMITMENT DEFENDED</td><td style="border-bottom:1px dashed rgba(212,168,67,0.2);text-align:right;font-size:22px;color:#D4A843">₹${(15000 * dayCount).toLocaleString('en-IN')}</td></tr>
...
async function preflight() {
 ...
 ? { ok: false, detail: `${d.error.code || ''} ${d.error.error_user_msg || d.error.message}`.trim() }: { ok: true, detail: '@' + d.username }
 ...
 ? { ok: true, detail: 'Storage write + public read OK' }: { ok: false, detail: `Public URL returned ${pub.status} — IG cannot fetch media` }
