...
(function() {
 ...
 fetch(url, { headers: { 'apikey': KEY, 'Authorization': 'Bearer ' + KEY } }).then(function(r) { return r.json(); }).then(function(data) { cb(null, Array.isArray(data) ? data: []); }).catch(function(e) { cb(e, []); });
 }
 ...
 ctx.font = '500 26px ' + MONO;
 ctx.fillStyle = T.dim;
 ctx.fillText(' PAID · STREAK CONTINUES', W / 2, sealY + 142);
 }
 ...
 ctx.fillStyle = T.text;
 ctx.fillText(s.made ? 'COMMITMENTD. STILL MINE.': ' GONE. TOMORROW I SHOW UP.', W / 2, 1584);
 ...
 ctx.fillText('✗ MISSED · PAID · STREAK CONTINUES', W / 2, 858);
 }

 ctx.font = '500 20px ' + MONO;
 ctx.fillStyle = T.dim;
 ctx.fillText('ON RECORD · EVERY DAY · NO END DATE', W / 2, 926);
 ...
 ctx.font = '500 20px ' + MONO;
 ctx.fillStyle = T.dim;
 ctx.fillText('/DAY LOCKED IN · DAY 1 — 13 JUN 2026', W / 2, 948);
 ...
 ctx.fillText(s.made ? '✓ BEFORE 6:00 AM': '✗ AFTER 6:00 AM — PAID', W / 2, sealY + 78);
 ...
 ctx.fillText('COMMITMENTD · STILL MINE · DAY ' + s.day + (dd ? ' — ' + dd: ''), W / 2, 948);
 ...
 renderBoth();
 captionEl.value = '0\n\n110 days. Then one morning, nothing.\nRestarting tomorrow — 5 km before 6 AM. Every day. daily commitment at stake.\n\nfirstlight.live';
 ...
 fetch(SUPA + '/rest/v1/proof_archive?date=eq.' + dateStr + '&select=food_clean', { headers: { 'apikey': KEY, 'Authorization': 'Bearer ' + KEY } }).then(function(r) { return r.json(); }).then(function(rows) { currentStats.foodClean = (rows && rows[0]) ? rows[0].food_clean!== false: true; }).catch(function() { currentStats.foodClean = true; }).then(function() {
 renderBoth();
 ...
 return uploadCanvasToStorage(canvas, filename).then(function(publicUrl) {
 ...
 }).then(function(containerId) { return igProxy(IG_ACCOUNT + '/media_publish', { creation_id: containerId }); }).then(function(pub) {
 ...
 publishCanvas(getCanvas(), getParams(), prefix + '_day' + currentStats.day + '_' + Date.now() + '.jpg').then(function(mediaId) {
